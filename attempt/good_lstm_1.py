...
#     model = tf.keras.Sequential()
    ...
    model = tf.keras.models.Sequential([
        ...
        #                        input_shape=[None]),
        tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(150,input_shape=(win_length, num_features), return_sequences=True)),
    ...
    model.compile(loss="mae", optimizer=tf.optimizers.Adam(), metrics=["accuracy"])
    # model.compile(loss="mae", optimizer=tf.keras.optimizers.SGD(lr=1e-6, momentum=0.9), metrics=["accuracy"])
    history = model.fit(train_generator, epochs=800, validation_data=test_generator, shuffle=False)#     model.add(tf.keras.layers.Dense(1, activation='sigmoid'))  # to check
#     model = tf.keras.Sequential()
#     model.add(tf.keras.layers.LSTM(10, input_shape=(win_length, num_features)))
#     model.add(tf.keras.layers.Dense(1, activation="linear"))
#     # compile model
#     model.compile(loss=tf.keras.losses.MeanSquaredError(),optimizer=tf.optimizers.Adam(), metrics=["accuracy"])
#     # fit model
#
#     history = model.fit(train_generator, epochs=800,validation_data=test_generator,shuffle=False)
#     print(model.summary())

    # model.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer=tf.optimizers.Adam(), metrics=["accuracy"])
    # history = model.fit(train_generator, epochs=650, validation_data=test_generator, shuffle=False)

    # history = model.fit(train_generator, epochs=100, verbose=1,validation_data=test_generator, shuffle=False)
